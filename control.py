...
def main(**kwargs):
    ...
    if kwargs['vae']:
        unet = VAEUNet(image_size=imsize,
                        ...
                        out_ch=channels,
                        ch=kwargs['dim'],
                        ch_mult= (1,) + (2,) * kwargs['num_downsamples'],
                        num_res_blocks=2,
    ...
    prior = torch.randn(kwargs['batch_size'], res, res).to(kwargs['device'])
    std = kwargs['prior_std']
    mean = kwargs['prior_mean']

    standard_prior = prior
    manipulated_prior = prior * std + mean
