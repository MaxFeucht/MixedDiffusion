...
parser.add_argument('--latent_dim', type=float, default=10, help='Which dimension the VAE latent space is supposed to have')
...
for t in tqdm(reversed(range(kwargs['timesteps'])), desc=f"Cold Sampling"):
    t_tensor = torch.full((kwargs['num_samples'],), t, dtype=torch.long).to(kwargs['device']) # t-1 to account for 0 indexing that the model is seeing during training
