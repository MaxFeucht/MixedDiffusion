...
#%%
    ...
    diff = 50
    t2 = torch.ones((kwargs['n_samples'],), dtype=torch.long).to(kwargs['device']) * kwargs['timesteps'] - diff
    ...
    for t in tqdm(reversed(range(kwargs['timesteps'])), desc=f"Cold Sampling"):
        ...
        t2 = torch.full((kwargs['n_samples'],), t-1, dtype=torch.long).to(kwargs['device']) # t-1 to account for 0 indexing that the model is seeing during training
