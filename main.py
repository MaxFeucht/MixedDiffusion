...
def main(**kwargs):
    ...
    # Training Loop
    for e in tqdm(range(kwargs['epochs']), total=kwargs['epochs']):
        
        val_flag = True if e % 10 == 0 else False
        ...
        for i, img in enumerate(samples):
            plt.imsave(f'./epoch_{e}_img_{i}.png', img)

    # Save model
    torch.save(trainer.model.state_dict(), f'./unet_{kwargs["dataset"]}.pt')
