...
def sample_z(mu_logvar = None, target = None, prior = None):
    ...
    if mu_logvar is not None:
        ...
        mu = mu_logvar[:,0,:,:].unsqueeze(1)
        logvar = mu_logvar[:,1,:,:].unsqueeze(1)
    ...
    # In Generation mode, we don't have the conditioning signal
        ...
        if prior is None:
            z_sample = th.randn(target.shape[0], target.shape[0]).to(target.device) # Batch dim * latent dim
        else:
            z_sample = prior.to(target.device)
...
class VAEUnet(nn.Module):
    ...
    def __init__(
        ...
        # VAE mean and logvar channels added to the input
        self.input_blocks = nn.ModuleList(
            [TimestepEmbedSequential(conv_nd(self.dims, in_channels, ch + 2, 3, padding=1,
                ...
                self.input_blocks.append(
                    TimestepEmbedSequential(
                        ResBlock(
                            ...
                            self.time_embed_dim,
                            dropout,
                            out_channels=out_ch + 2,  # +2 for mean and logvar
                            dims=self.dims,
                            use_checkpoint=use_checkpoint,
                            use_scale_shift_norm=self.use_scale_shift_norm,
                            down=True,
                        ...
                        if self.resblock_updown
                            ...
                            ch, self.conv_resample, dims=self.dims, out_channels=out_ch + 2, # +2 for mean and logvar
                            padding_mode=self.padding_mode
                        )
                    )
                )
        ...
        for level, mult in list(enumerate(self.channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                cond = level != len(self.channel_mult)-1
                channel_add = self.model_channels #(int(ch/self.model_channels) + int(ich/self.model_channels)) # if cond else 0
                #print(i, cond, len(self.channel_mult))
                layers = [
                    ResBlock(
                        (ch + ich + channel_add) if i == 0 else (ch + ich), # + mult * 2 for the z-sample that's repeated for mult amount of times
                        self.time_embed_dim,
                        dropout,
        ...
        self.out = nn.Sequential(
            normalization(ch, num_groups=32),
            nn.SiLU(),
            zero_module(conv_nd(self.dims, input_ch, self.out_channels, 3,
    ...
    def forward(self, xt, t, cond=None, prior=None, t2 = None, y=None):
        ...
        for i, module in enumerate(self.input_blocks):
            ...
            if (i % (self.num_res_blocks+1)) == 0:
                mean_logvar = h[:,-2:,:,:] # Extract the mean and logvar channels
                z, kl = sample_z(mean_logvar) # Sample from mean and logvar to get z
                latents.append(z)
                kls.append(kl)
                mult = self.model_channels # Get the multiplier for the z-sample
                zs.append(z.repeat(1, mult, 1, 1)) # Repeat the z-sample to match the feature map dimensions
                h = h[:,:-2,:,:]
        ...
        for j, module in enumerate(self.output_blocks):
            ...
            if j % (self.num_res_blocks+1) == 0:
                h = th.cat([h, zs.pop()], dim=1) # Append the VAE latent to the feature maps

            h = module(h, emb)

        #self.kl_div = sum(self.kls).detach()
        self.kl_div = th.tensor(kls).sum()
        self.kls = kls
