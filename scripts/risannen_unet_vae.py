...
class VAEUnet(nn.Module):
    ...
    def __init__(
        ...
        assert vae_loc in ['start', 'bottleneck', 'emb', 'maps'], 'VAE location must be one of [start, bottleneck, emb, maps]'
        ...
        if self.vae_loc == 'emb' and self.vae_inject == 'concat':
            self.time_embed_dim += self.latent_dim
    ...
    def setup_vae_projection(self):
        ...
        if not self.vae_loc == 'maps': # Different treatment for when injection happens at feature maps
            if self.vae_loc == 'start':
                ...
                target_dim = self.time_embed_dim
            ...
            elif self.vae_loc == 'maps':
                target_dim = self.image_size # Doesn't matter, isn't used
            else:
                raise ValueError('VAE Injection point invalid')
            
            # Project latent from latent dim to U-Net Dim, depending on injection point
            self.vae_projection = nn.Linear(self.latent_dim, target_dim)
        ...
        # Unique projection for each downsample step in additive injection

        if self.vae_loc == "maps":
            self.vae_projections.append(nn.Linear(self.latent_dim, self.in_channels*curr_res*curr_res))
    ...
    def vae_injection(self, target, z_sample, index = None):
        ...
        if self.vae_inject == 'add':
            ...
            if self.vae_loc == 'emb':
                ...
                injection = injection.view(target.shape[0], target.shape[1])
            else:
    ...
    def forward(self, xt, t, cond=None, prior=None, t2 = None, y=None):
        ...
        z_sample, self.kl_div = vae_encoding(self.vae_encoder, 
        ...
        for i, module in enumerate(self.input_blocks):
            ...
            if self.vae_loc == "maps" and i > 0:
                h = self.vae_injection(h, z_sample, index = i)
